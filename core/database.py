...
# ─────────────────────────────────────────────────────────────────────────────
# Scraper Intelligence System — Lead discovery and qualification
# ─────────────────────────────────────────────────────────────────────────────

class ScraperRun(TimestampMixin, Base):
    """A scraper execution session — tracks progress, providers, and results."""
    __tablename__ = "scraper_runs"

    id = Column(String, primary_key=True, index=True)
    status = Column(String, default="pending")  # pending/running/completed/failed/cancelled
    providers = Column(JSON, default=list)  # ["producthunt", "hackernews"]
    ...
    leads = relationship("ScraperLead", back_populates="run", cascade="all, delete-orphan")
    logs = relationship("ScraperLog", back_populates="run", cascade="all, delete-orphan")

    __table_args__ = (
        Index('ix_scraper_runs_status', 'status', 'created_at'),
        Index('ix_scraper_runs_owner', 'owner', 'created_at'),
    )


class ScraperLead(TimestampMixin, Base):
    """A discovered startup lead with contact info and AI scoring."""
    __tablename__ = "scraper_leads"

    id = Column(String, primary_key=True, index=True)
    run_id = Column(String, ForeignKey("scraper_runs.id", ondelete="SET NULL"), nullable=True, index=True)
    # Core data
    name = Column(String, nullable=False)
    website = Column(String, nullable=True)
    description = Column(Text, nullable=True)
    domain = Column(String, nullable=True, index=True)  # for dedupe
    # Source
    ...
    promo_video_fit_score = Column(Integer, nullable=True)  # 0-100
    urgency_score = Column(Integer, nullable=True)
    funding_probability = Column(Integer, nullable=True)
    ai_summary = Column(Text, nullable=True)
    ai_reasoning = Column(Text, nullable=True)
    outreach_recommendations = Column(Text, nullable=True)
    # Status
    ...
    run = relationship("ScraperRun", back_populates="leads")

    __table_args__ = (
        Index('ix_scraper_leads_domain_unique', 'domain'),
        Index('ix_scraper_leads_scores', 'affordability_score', 'promo_video_fit_score'),
        Index('ix_scraper_leads_not_excluded', 'excluded', 'created_at'),
    )


class ScraperLog(TimestampMixin, Base):
    """Log entry for scraper operations."""
    __tablename__ = "scraper_logs"

    id = Column(String, primary_key=True, index=True)
    run_id = Column(String, ForeignKey("scraper_runs.id", ondelete="CASCADE"), nullable=True, index=True)
    level = Column(String, default="info")  # info/warn/error/debug
    provider = Column(String, nullable=True)
    message = Column(Text, nullable=False)
    data = Column(JSON, nullable=True)

    run = relationship("ScraperRun", back_populates="logs")


class ScraperProvider(Base):
    """Configuration for a scraper data source."""
    __tablename__ = "scraper_providers"

    id = Column(String, primary_key=True, index=True)
    name = Column(String, nullable=False, unique=True)
    enabled = Column(Boolean, default=True)
    config = Column(JSON, default=dict)  # rate limits, selectors, etc.
    last_run_at = Column(DateTime, nullable=True)
    stats = Column(JSON, default=dict)  # {"total_scraped": 0, "success_rate": 0}
